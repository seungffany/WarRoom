...
<style>
    ...
    .header-text {
        ...
        text-shadow: 0 0 10px rgba(194, 165, 109, 0.5); letter-spacing: 5px; 
        border-bottom: 2px solid #C2A56D; padding: 20px 0; margin-bottom: 10px; 
    ...
    .ai-market-label { font-family: 'Orbitron'; font-size: 1.1rem; color: #C2A56D; margin-bottom: 10px; font-weight: 700; }
    .ai-content { color: #d1d1d1; font-size: 1.1rem; line-height: 1.6; }
    ...
    .news-ticker-container {
        ...
        background: #1A242D; padding: 15px 0; overflow: hidden; white-space: nowrap; margin-bottom: 10px;
    }
    .news-ticker-content {
        display: inline-block; padding-left: 100%; animation: ticker 200s linear infinite;
    }
    .news-item-inline { display: inline-block; margin-right: 100px; font-size: 1.3rem; color: #E8EDF2; font-weight: 500; }
    @keyframes ticker { 0% { transform: translate3d(0, 0, 0); } 100% { transform: translate3d(-100%, 0, 0); } }
    
    .news-list-item {
        padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.05); color: #ccc; font-size: 1.1rem;
    }
    .news-list-item:hover { color: #C2A56D; }
...
def fetch_expanded_news():
    ...
    except: return ["데이터 링크 불안정. 백업 시스템 가동..."]

def get_simple_tactical_report(market_name, ticker, news):
    """지표 기반의 심플한 전술 분석 리포트를 생성합니다. (오류 방지형)"""
    ...
    <div class="ai-content">
        보고: {market_label} 분석 완료.<br>
        현재가는 {curr:,.2f}로 MA20 대비 <span class="{status_class}">{trend} 추세</span>입니다. <br>
        변동률 {change:+.2f}% 감지. 전술적 매수세 유입 여부를 모니터링 중입니다.
    </div>
    """
    return report
...
def render_metric_card(name, df, unit):
    ...
    <div class="metric-card-custom">
        ...
        <div style="font-size:2.2rem; font-weight:800; color:#E8EDF2; margin: 8px 0;">{p_str}<span style="font-size:1.1rem; color:#888;"> {display_unit}</span></div>
        <div style="color:{color}; font-size:1.3rem; font-weight:700;">{"▼" if delta < 0 else "▲"} {abs(delta):.2f}%</div>
    </div>
    """, unsafe_allow_html=True)
...
# [심플화] 3대 시장 전술 분석 섹션
st.markdown('<div class="section-container"><div class="section-title">TACTICAL INTELLIGENCE BRIEFING (LOCAL_AI)</div>', unsafe_allow_html=True)
ai_cols = st.columns(3)

with ai_cols[0]:
    analysis_us = get_simple_tactical_report("US", "^GSPC", expanded_news)
    st.markdown(f'<div class="ai-briefing-box"><div class="ai-market-label">🇺🇸 US_THEATER</div>{analysis_us}</div>', unsafe_allow_html=True)

with ai_cols[1]:
    analysis_kr = get_simple_tactical_report("KR", "KS11", expanded_news)
    st.markdown(f'<div class="ai-briefing-box"><div class="ai-market-label">🇰🇷 KR_THEATER</div>{analysis_kr}</div>', unsafe_allow_html=True)

with ai_cols[2]:
    analysis_jp = get_simple_tactical_report("JP", "^N225", expanded_news)
...
with c1_gauge:
    ...
    fig_gauge.add_trace(go.Indicator(
        ...
        gauge = {'axis': {'range': [0, 100], 'tickfont': {'size': 18}}, 'bar': {'color': "rgba(0,0,0,0)"}, 'bgcolor': "rgba(0,0,0,0)",
    ...
    fig_gauge.update_layout(paper_bgcolor='rgba(0,0,0,0)', font={'color': "#E8EDF2", 'family': "Orbitron", 'size': 16}, 
                            height=250, margin=dict(l=20, r=20, t=40, b=0))
    st.plotly_chart(fig_gauge, width='stretch')
with c2_info:
    st.markdown("<br><br>", unsafe_allow_html=True)
    st.markdown(f"<div style='border-left: 5px solid #C2A56D; padding: 15px; background: rgba(194, 165, 109, 0.05); font-size:1.4rem;'>현재 시장 공포-탐욕 지수는 <b>{score}pts</b>입니다. <br>심리적 과열 혹은 위축 상태를 전술 분석과 대조하여 판단하십시오.</div>", unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)

# 종목 관리 기능
with st.expander("🛠️ COMMAND CONFIGURATION (ADD/DELETE ASSETS)"):
    tab1, tab2 = st.tabs(["Add Asset", "Delete Asset"])
    with tab1:
        c_add1, c_add2, c_add3, c_add4 = st.columns([2, 2, 1, 1])
        new_name = c_add1.text_input("Asset Name (e.g. BTC)")
        new_ticker = c_add2.text_input("Ticker Symbol (e.g. BTC/USD)")
        category = c_add3.selectbox("Category", ["Economy", "Index", "Stock"])
        suggested_unit = "pts" if "^" in new_ticker or "KS11" in new_ticker else ("KRW" if new_ticker.isdigit() else "USD")
        new_unit = c_add4.selectbox("Unit", ["USD", "KRW", "JPY", "pts", "%"], index=["USD", "KRW", "JPY", "pts", "%"].index(suggested_unit) if suggested_unit in ["USD", "KRW", "pts"] else 0)
        if st.button("DEPLOY NEW ASSET", width='stretch'):
    ...
    with tab2:
        del_target = st.selectbox("Select Asset to Remove", list(all_targets.keys()))
        if st.button("TERMINATE ASSET NODE", width='stretch'):
...
with cc1: sel_target = st.selectbox("CHOOSE NODE", list(all_targets.keys()), label_visibility="collapsed")
with cc2: date_range = st.select_slider("TIME RANGE (DAYS)", options=[7, 14, 30, 60, 90, 180, 365], value=30, label_visibility="collapsed")
with cc3: 
    ...
    if target_df is not None:
        ...
        st.markdown(f"<div style='text-align:right; font-family:Orbitron; font-weight:800; color:{t_col}; font-size:1.5rem;'>STATUS: {t_sent}</div>", unsafe_allow_html=True)

if target_df is not None:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=target_df.index, y=target_df['close'], name="Price", line=dict(color='#E8EDF2', width=3)))
    ...
    fig.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                      height=450, margin=dict(l=0, r=0, t=20, b=0), xaxis=dict(showgrid=False, tickfont=dict(size=14)),
